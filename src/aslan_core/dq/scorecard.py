...
def render_html(*, week_start: date, rows: list[ScorecardRow]) -> str:
    ...
    Companion to :func:`render_pdf`, which renders this HTML through
    WeasyPrint to PDF bytes when the optional ``[obs]`` extra is
    installed.
    """
    pass_pct = _summary_pass_pct(rows)
    pass_n = sum(1 for r in rows if r.status == "pass")
...
def render_pdf(*, week_start: date, rows: list[ScorecardRow]) -> bytes:
    """Render the standalone scorecard HTML to PDF bytes via WeasyPrint.

    WeasyPrint is an optional dependency installed via the
    ``aslan-core[obs]`` extra (it pulls in native libs — pango, cairo,
    gdk-pixbuf — that the base install intentionally avoids). The
    import is performed lazily so a base aslan-core install runs
    without WeasyPrint and only callers that actually need PDF export
    pay the dependency cost.

    Hetzner deployment note: the production image must
    ``apt install libpango-1.0-0 libpangoft2-1.0-0 libharfbuzz-subset0``
    so WeasyPrint's native font/text-shaping pipeline finds its libs.

    Raises:
      RuntimeError: WeasyPrint is not installed. The error message
        points at the ``aslan-core[obs]`` extra so the operator knows
        the supported install path.
    """
    try:
        import weasyprint
    except ImportError as exc:  # pragma: no cover - exercised in skip-on-missing test
        raise RuntimeError(
            "WeasyPrint not installed; install aslan-core[obs] to enable PDF rendering."
        ) from exc
    html = render_html(week_start=week_start, rows=rows)
